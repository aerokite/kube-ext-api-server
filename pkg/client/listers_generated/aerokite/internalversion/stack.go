...
type StackLister interface {
	...
	List(selector labels.Selector) (ret []*aerokite.Stack, err error)
...
// List lists all Stacks in the indexer.
func (s *stackLister) List(selector labels.Selector) (ret []*aerokite.Stack, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*aerokite.Stack))
	})
	return ret, err
}
...
type StackNamespaceLister interface {
	...
	Get(name string) (*aerokite.Stack, error)
...
// List lists all Stacks in the indexer for a given namespace.
func (s stackNamespaceLister) List(selector labels.Selector) (ret []*aerokite.Stack, err error) {
...
// Get retrieves the Stack from the indexer for a given namespace and name.
func (s stackNamespaceLister) Get(name string) (*aerokite.Stack, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(aerokite.Resource("stack"), name)
	}
	return obj.(*aerokite.Stack), nil
}
